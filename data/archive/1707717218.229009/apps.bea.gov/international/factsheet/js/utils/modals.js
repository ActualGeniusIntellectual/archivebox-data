...
const modals = new (function () {
    ...
    this.init = function () {
        if (wait === null) {
            var waitModal = new obj.ModalObj('pleaseWait-modal');
        ...
        $(document).on('App-Loaded', function () {
            // console.log('in modals - app-loaded event triggered');
            modals.Wait('hide');
        });

        obj.finishedLoading = true;
    };

    this.Wait = function (action) {
        clearTimeout(obj.closeModalTimeout);
        clearTimeout(obj.clearGrayScreen);
        if (action == 'show') {
            wait.modal.modal('show');
        } else {
            obj.closeModalTimeout = setTimeout(function () {
                wait.modal.modal('hide');
            }, 500);
        }
        obj.clearGrayScreen = setTimeout(function () {
    ...
    this.MakeModal = function (o) {
        ...
        var close = $(
            '<span class="btn close text-danger closeButton" data-dismiss="modal" aria-hidden="true">&times;</span>'
        );
